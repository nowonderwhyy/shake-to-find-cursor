...
public static class CursorHelper
{
    ...
    // Guards _scaleFrames, _frameScales, IsCached and _lastFactor against the
    // animation loop (ApplyScaleFrame/GetFrameIndexForScale) racing a rebuild in InitCaches.
    ...
    {
        // Cheap guard: skip the rebuild if this factor is already cached.
        lock (_sync)
        {
            if (IsCached && Math.Abs(_lastFactor - magnificationFactor) < 1e-3) return;
        }

        // Build the new frame set into locals OUTSIDE the lock. This is the slow part
        // (per-cursor LoadImage/GDI work) and must not block the animation loop.
        // Build non-linear scale table: power bias packs more frames near 1.0
        double peakScale = magnificationFactor * 1.05;
        var newScales = new double[ScaleSteps];
        for (int i = 0; i < ScaleSteps; i++)
        {
            double t = i / (double)(ScaleSteps - 1);
            double biased = Math.Pow(t, 1.8); // ~60% of frames below 1.25x
            newScales[i] = 1.0 + ((peakScale - 1.0) * biased);
        }

        var newFrames = new Dictionary<uint, IntPtr[]>();
        ...
        using (var key = Registry.CurrentUser.OpenSubKey(@"Control Panel\Cursors"))
        {
            foreach (uint id in SYSTEM_CURSORS)
            {
                ...
                for (int i = 0; i < ScaleSteps; i++)
                {
                    frames[i] = GenerateFrame(path, fallbackSysHCursor, newScales[i]);
                }
        ...
        // Publish the new set and destroy the old one atomically. All reads of the
        // frame handles happen under _sync, so destroying the old handles here is safe.
        lock (_sync)
        {
            var oldFrames = _scaleFrames;

            _scaleFrames = newFrames;
            _frameScales = newScales;
            _lastFactor = magnificationFactor;
            IsCached = true;

            foreach (var arr in oldFrames.Values)
    ...
    public static void ApplyScaleFrame(int frameIndex)
    {
        try
        {
            ...
            try
            {
                // Hold the lock across the whole loop so CopyIcon never runs on a
                // handle that InitCaches is concurrently destroying.
                lock (_sync)
                {
